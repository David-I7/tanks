...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    function animate(timestamp) {
        ...
        if (mouseGestures.wasReleased()) {
            console.log(`Released: ${JSON.stringify(mouseGestures.getMouseRelease())}`);
        }
        if (mouseGestures.wasDoubleClicked()) {
            console.log(`Double CLick: ${JSON.stringify(mouseGestures.getDoubleClick())}`);
        }
        // Mouse events
        if (mouseGestures.hasMouseMoved()) {
            console.log(`Mouse moved: ${JSON.stringify(mouseGestures.getMouseMoves())}`);
        }
        if (mouseGestures.hasMouseLeft()) {
            console.log(`Mouse left: ${JSON.stringify(mouseGestures.getMouseLeaves())}`);
        }
        // Keyboard
        ...
        if (keyboardGestures.getPressedKeys().length) {
            console.log(`Keys pressed: ${JSON.stringify(keyboardGestures.getPressedKeys())}`);
        }
        if (keyboardGestures.getTypedKeys().length) {
            console.log(`Keys typed: ${JSON.stringify(keyboardGestures.getTypedKeys())}`);
        }
        if (keyboardGestures.getReleasedKeys().length) {
            console.log(`Keys released: ${JSON.stringify(keyboardGestures.getReleasedKeys())}`);
        }
        if (keyboardGestures.getKeysHeld().size) {
            console.log(`Keys held: ${JSON.stringify([...keyboardGestures.getKeysHeld()])}`);
        }
