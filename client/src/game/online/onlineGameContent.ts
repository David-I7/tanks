...
export function onlineGameContentFromResponse(response: GameContentResponseDto): GameContent {
  return {
    ...
    tanks: Object.fromEntries(Object.entries(response.tanks).map(([id, tank]) => [id, {
      id: tank.id,
      name: tank.name,
      ...
      loadout: tank.loadout.map(({ id: slotId, projectileDefinitionId, label, renderAssetId }) => ({
        id: slotId, projectileDefinitionId, label, renderAssetId,
      })),
      visual: visualFor(tank.renderAssetId),
    }])),
    projectiles: Object.fromEntries(Object.entries(response.projectiles).map(([id, projectile]) => [id, {
      id: projectile.id,
      ...
      terrainEffect: projectile.terrainEffectType === "CRATER"
        ? { type: "crater" as const, radius: projectile.terrainRadius }
        : { type: "drill" as const, radius: projectile.terrainRadius, depth: projectile.terrainDepth },
      damageEffect: projectile.damageEffectType === "RADIAL"
        ? { type: "radial" as const, radius: projectile.damageRadius, damage: projectile.damage }
        : { type: "focused" as const, radius: projectile.damageRadius, damage: projectile.damage },
...
function visualFor(renderAssetId: string): VisualIdentity {
  const known: Record<string, VisualIdentity> = {
    "tank.vanguard": { fill: "#22c55e", stroke: "#14532d", accent: "#bbf7d0", label: "V" },
    "tank.specter": { fill: "#60a5fa", stroke: "#1e3a8a", accent: "#dbeafe", label: "S" },
    "projectile.basic-shell": { fill: "#f8fafc", stroke: "#f97316", accent: "#fed7aa", label: "S" },
    "projectile.mortar": { fill: "#a3a3a3", stroke: "#404040", accent: "#e5e5e5", label: "M" },
    "projectile.heavy-shell": { fill: "#fca5a5", stroke: "#991b1b", accent: "#fee2e2", label: "H" },
    "projectile.cluster": { fill: "#fde047", stroke: "#854d0e", accent: "#fef9c3", label: "C" },
    "projectile.needle": { fill: "#93c5fd", stroke: "#1e40af", accent: "#dbeafe", label: "N" },
  };
