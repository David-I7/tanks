import CanvasDragEvent from "../events/canvas/CanvasDragEvent";
import CanvasMouseEvent from "../events/canvas/CanvasMouseEvent";
import CanvasWheelEvent from "../events/canvas/CanvasWheelEvents";
import { InputEventBuffer } from "../events/InputEventBuffer";
import Camera from "../ui/Camera";
import ViewportInput from "./ViewportInput";
import Viewport from "./ViewportInput";
...
export class MouseGestures {
  ...
  collect(eventBuffer: InputEventBuffer) {
    if (this.hasDragStarted()) {
      eventBuffer.push(this.getDragStart()!);
    }

    if (this.hasDragMoved()) {
      eventBuffer.push(...this.getDragMoves());
    }

    if (this.hasDragEnded()) {
      eventBuffer.push(this.getDragEnd()!);
    }

    if (this.hasMouseMoved()) {
      eventBuffer.push(...this.getMouseMoves());
    }

    if (this.hasMouseLeft()) {
      eventBuffer.push(...this.getMouseLeaves());
    }

    if (this.hasWheelMoved()) {
      eventBuffer.push(...this.getWheelMoves());
    }

    if (this.wasClicked()) {
      eventBuffer.push(this.getClick()!);
    }

    if (this.wasDoubleClicked()) {
      eventBuffer.push(this.getDoubleClick()!);
    }

    if (this.wasPressed()) {
      eventBuffer.push(this.getMousePress()!);
    }

    if (this.wasReleased()) {
      eventBuffer.push(this.getMouseRelease()!);
    }
  }
}
